...
async def test_SCS(browser_args):
    """
    为 test_SCS 创建完全独立的浏览器实例，使用指定的浏览器参数
    """
    task_id = "test_SCS"
    async with async_playwright() as p:
        ...
        try:
            ...
            if await ui_operations.is_browser_closed():
                log_info(f"[{task_id}] 检测到浏览器已关闭，test_SCS 测试无法继续")
            ...
            with allure.step("测试步骤1: 点击开始按钮"):
                ...
                # 步骤后截图
                with allure.step("测试步骤1: 步骤后截图"):
                    await ui_operations.page_screenshot(f"test_SCS","test_step_1_after")
            ...
            with allure.step("测试步骤2: 点击同意按钮"):
                ...
                # 步骤后截图
                with allure.step("测试步骤2: 步骤后截图"):
                    await ui_operations.page_screenshot(f"test_SCS","test_step_2_after")
            ...
            with allure.step("测试步骤3: 点击图片，进入游戏"):
                ...
                with allure.step(f"测试步骤3: 点击图片，进入游戏 - 游戏图片click 操作"):
                    ...
                    for attempt in range(1):
                        ...
                        if await ui_operations.is_browser_closed():
                            log_info(f"[{task_id}] 检测到浏览器已关闭，test_SCS 测试被用户中断")
                            raise Exception("BROWSER_CLOSED_BY_USER")
                        
                        try:
                            log_info(f"[test_SCS] 执行第{attempt + 1}次图片操作: click on Game_Img/1758183263_Snipaste_2025-09-01_15-59-25.png")
                            time.sleep(3)
                            success = await ui_operations.click_image_with_fallback(
                                "Game_Img/1758183263_Snipaste_2025-09-01_15-59-25.png", 
                                confidence=0.5, 
                                timeout=10
                            )
                            if success:
                                log_info(f"[{task_id}] test_SCS 第{attempt + 1}次操作完成")
                            else:
                                log_info(f"[{task_id}] test_SCS 第{attempt + 1}次尝试：没有找到图片 Game_Img/1758183263_Snipaste_2025-09-01_15-59-25.png")
                                if attempt == 1 - 1:  # 最后一次尝试失败
                                    log_info(f"[{task_id}] test_SCS 所有 1 次尝试都失败，无法找到图片")
                                raise Exception(f"图片定位失败：无法找到图片 Game_Img/1758183263_Snipaste_2025-09-01_15-59-25.png")
                        except Exception as e:
                            log_info(f"[{task_id}] test_SCS 第{attempt + 1}次图片定位失败")
                            if attempt == 1 - 1:  # 最后一次尝试失败
                                log_info(f"[{task_id}] test_SCS 所有 1 次尝试都失败")
                            raise Exception(f"图片定位失败：无法找到图片 Game_Img/1758183263_Snipaste_2025-09-01_15-59-25.png")
                        time.sleep(1)  # 每次操作后等待1秒
            ...
            # 最终检查浏览器状态
            if await ui_operations.is_browser_closed():
                log_info(f"检测到浏览器已关闭，test_SCS 测试被用户中断")
                raise Exception("BROWSER_CLOSED_BY_USER")
            
            await ui_operations.page_screenshot(f"test_SCS","test_step_3")
            time.sleep(2)
            ...
            log_info(f"[{task_id}] test_SCS 完成")
            
        except Exception as e:
            log_info(f"[{task_id}] test_SCS 失败")
            raise e
        finally:
            # 清理资源
            if page:
...
async def test_concurrent_independent_browsers():
    ...
    try:
        ...
        # 创建 1 个独立的浏览器实例任务
        task1 = asyncio.create_task(test_SCS(browser_args_list[0]))
